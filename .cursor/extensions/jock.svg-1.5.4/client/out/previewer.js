...
exports.SvgPreviwerContentProvider = exports.AllSvgPreviwerContentProvider = exports.registerPreviewer = void 0;
...
function getPreviewConfiguration() {
    return vscode.workspace.getConfiguration('svg').get('preview', DefaultPreviewConfig);
}
let customCssFiles = [];
function onDidChangeActiveTextEditor(e, show) {
...
function registerPreviewer() {
    SvgPreviwerContentProvider.$context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(e => onDidChangeActiveTextEditor(e)), vscode.commands.registerTextEditorCommand('svg.showSvg', () => show()), vscode.commands.registerCommand('_svg.showSvgByUri', uri => show(uri)), new AllSvgPreviwerContentProvider());
}
exports.registerPreviewer = registerPreviewer;
function getOrCreateDocumentEditor(uri, options) {
...
class AllSvgPreviwerContentProvider {
    constructor() {
        this.subdisposed = [];
        this.subdisposed.push(vscode.commands.registerCommand('svg.showAllSvg', () => this.showAllSvg()));
    }
    showAllSvg() {
        ...
        if (this.webviewPanel == null) {
            this.webviewPanel = vscode.window.createWebviewPanel('all-svg-preview', 'Preview All SVG', {
...
exports.AllSvgPreviwerContentProvider = AllSvgPreviwerContentProvider;
class SvgPreviwerContentProvider {
     ...
     */
    constructor() {
        this.isRootLocked = false;
        this.isLocked = false;
        // path: vscode.Uri;
        this.noSaveBackground = null;
        this.showRuler = undefined;
        this.showCrossLine = undefined;
    ...
    show(e) {
        if (this.webviewPanel == null) {
            ...
            this.webviewPanel = vscode.window.createWebviewPanel('svg-preview', 'Svg Preview', {
                viewColumn: vscode.ViewColumn.Three,
                preserveFocus: true
            }, { enableScripts: true });
            this.webviewPanel.webview.onDidReceiveMessage(e => this.onDidReceiveMessage(e));
    ...
    onDidReceiveMessage(e) {
        switch (e.action) {
            case 'bg':
                let saveTo = vscode.workspace.getConfiguration('svg.preview').get('backgroundSaveTo', 'Workspace');
            ...
            case 'export':
                {
                    if (e.b64) {
                        // let b = <Blob>e.blob;
                        let uri = vscode.Uri.parse(e.uri);
                        let newUri = unit_1.changeName(uri, (n, e) => n + '.png');
                        vscode.window.showSaveDialog({
                            ...
                            if (r) {
                                unit_1.writeB64ToFile(e.b64, r.fsPath, e => {
                                    if (e) {
    ...
    reshow() {
        console_1.assert(this.previewUri != null, "Use reshow() required previewUri is not null.");
    ...
    showDocument(doc) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.webviewPanel.webview.html = yield this.createHtml(doc, this.webviewPanel.webview);
        });
    }
    getDefined(...options) {
    ...
    createHtml(doc, webivew) {
        return __awaiter(this, void 0, void 0, function* () {
            const fileSettings = getFileSettings(this.previewUri);
            let cfg = vscode.workspace.getConfiguration('svg.preview');
            let saveTo = cfg.get('backgroundSaveTo', 'Workspace');
            let toolbarSize = cfg.get('toolbarSize', 'mini');
            let autoFit = cfg.get("autoFit", true);
            let scaleZoom = cfg.get("scaleZoom", DefaultPreviewConfig.scaleZoom);
            let zoomOptions = cfg.get("zoomOptions", DefaultPreviewConfig.zoomOptions);
            let translateExternalAddress = cfg.get("translateExternalAddress", false);
            ...
            let bg = (saveTo == 'NoSave' && this.noSaveBackground) || cfg.get('background') || 'transparent';
            let bgCustom = cfg.get('backgroundCustom') || '#eee';
            let viewMode = cfg.get('viewMode', 'onlyOne');
            let showRuler = this.getDefined(this.showRuler, cfg.get('showRuler', false));
            let showCrossLine = this.getDefined(this.showCrossLine, cfg.get('showCrossLine', false));
            let mode = cfg.get('mode', 'svg');
            // let svg = doc.getText();
            let svg = mode === 'img' ? doc.getText() : this.converToInspectSvg(doc);
            svg = yield this.localImageHrefConvert(webivew, svg);
            let domains = [];
            let imageHrefs = [];
            let imageHrefReg = /\<(fe)?image([^>]*)href=\"(https?:\/\/[^\"]*)\"/gi;
            let imageHrefMatch = null;
            try {
                const translateImageMaps = {};
                while (imageHrefMatch = imageHrefReg.exec(svg)) {
                    ...
                    if (translateExternalAddress) {
                        try {
                            const contentBytes = yield new Promise((resolve, reject) => {
                                (/^https/i.test(imageHref) ? https : http).get(imageHref, res => {
                                    ...
                                    if (statusCode === 200) {
                                        ...
                                        res.on('end', () => {
                                            const buffer = isText ? Buffer.from(all.join('')) : Buffer.concat(all);
    ...
    localImageHrefConvert(webivew, svg) {
        return __awaiter(this, void 0, void 0, function* () {
